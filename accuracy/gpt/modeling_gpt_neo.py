...
class GPTNeoXAttention(nn.Module):
    ...
    def __init__(self, config, attention_type):
        ...
        attn_dropout_p = float(config.attention_dropout) if self.training else 0.0
        self.e_attn = FavorAttention(causal=self.is_causal, dropout=attn_dropout_p, dim_features=self.embed_dim, dim_head=self.head_dim)
    ...
    def forward(
        ...
        # attn_output = self.e_attn(query, key, value, attention_mask, head_mask)
        attn_output = self.e_attn(query, key, value)
...
class GPTNeoMAttention(nn.Module):
    """
    GPTNeo Favor attention module. 
    """
    def __init__(self, config, attention_type):
        ...
        self.ite = 0
        self.fast_attention = FastAttention(self.head_dim, nb_features=None, causal=self.is_causal)
    ...
    def forward(
        ...
        query_layer = self._split_heads(q_linear, self.num_linear_heads, self.head_dim)
        key_layer = self._split_heads(k_linear, self.num_linear_heads, self.head_dim)
        value_layer = self._split_heads(v_linear, self.num_linear_heads, self.head_dim)

        sparse_context = self.e_attn(q_sparse, k_sparse, v_sparse)
        context_layer = self.fast_attention(query_layer, key_layer, value_layer)
        context_layer = context_layer.permute(0, 2, 1, 3).contiguous()
        new_context_layer_shape = context_layer.size()[:-2] + (self.num_linear_heads*self.head_dim, )
        context_layer = context_layer.view(new_context_layer_shape)
        context_layer = torch.cat((sparse_context, context_layer), dim=-1)
...
class GPTNeoAttention(nn.Module):
    def __init__(self, config, layer_id=0):
        ...
        if self.attention_type in ["global", "local"]:
            ...
            # self.attention = GPTNeoXAttention(config, self.attention_type)
            # self.attention = GPTNeoMAttention(config, self.attention_type)
        else:
